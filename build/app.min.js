var _Mathround=Math.round;(function(){window.api=function(b,c){let d=new XMLHttpRequest;d.onreadystatechange=function(){let f={};if(4===d.readyState){if(200!==d.status)return void c(d.status);try{f=JSON.parse(d.responseText)}catch(g){return void c(g)}c(null,f)}},d.open('GET',b,!0),d.send(null)}})(),function(){let b=function(c){return document.getElementById(c)};window.api('https://api.github.com/repos/thepracticaldev/1pr/contributors',function(c,d){if(null!==c)return console.error(c),b('contributors').className='error',void(b('contributors').textContent='Could not load contributors');for(let f,e=0;e<d.length;e++){f=document.createElement('a'),f.href=d[e].html_url,f.title=d[e].login,f.target='_blank';let g=document.createElement('img');g.src=d[e].avatar_url,f.appendChild(g),b('contributors').appendChild(f)}b('contributors-container').style.display='block'})}(),((window.gitter={}).chat={}).options={room:'thepracticaldev/1pr',useStyles:!1};function toggleSidebar(){sidebar=document.getElementById('sidebar'),sidebar.style.display='none'==sidebar.style.display?'block':'block'==sidebar.style.display?'none':'block'}(function(){let b=function(e){return document.getElementById(e)},c=function(e){let f=new Date-new Date(e),g=_Mathround(f/1e3);if(60>g)return g+' seconds ago';let h=_Mathround(g/60);if(60>h)return h+' minutes ago';let j=_Mathround(h/60);if(24>j)return j+' hours ago';let k=_Mathround(j/24);return k+' days ago'},d=function(e,f){if(e.length<f)return e;let g=e.split(' ');e='';for(let j=0;j<g.length;j++){if(e+=g[j],e.length>=f){e+='...';break}e+=' '}return e};window.api('https://api.github.com/repos/thepracticaldev/1pr/pulls?state=closed',function(e,f){if(e)return console.error(e),b('pr-container').textContent='Could not load latest PR',void(b('pr-container').className='error');for(let g=0;g<f.length;g++)if(null!=f[g].merged_at){let h=f[g];b('pr-last').innerHTML='<a href="'+h.html_url+'">'+d(h.title,50)+'</a> by <a href="'+h.user.html_url+'">'+h.user.login+'</a> merged '+c(h.merged_at);break}b('pr-container').style.display='block'})})();
/*!

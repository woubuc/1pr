var _Mathabs=Math.abs,_Mathround=Math.round,_Mathtan=Math.tan,_MathPI=Math.PI;(function(){window.api=function(d,e){let f=new XMLHttpRequest;f.onreadystatechange=function(){let h={};if(4===f.readyState){if(200!==f.status)return void e(f.status);try{h=JSON.parse(f.responseText)}catch(k){return void e(k)}e(null,h)}},f.open('GET',d,!0),f.send(null)}})(),function(){let d=function(e){return document.getElementById(e)};window.api('https://api.github.com/repos/thepracticaldev/1pr/contributors',function(e,f){if(null!==e)return console.error(e),d('contributors').className='error',void(d('contributors').textContent='Could not load contributors');for(let h,g=0;g<f.length;g++){h=document.createElement('a'),h.href=f[g].html_url,h.title=f[g].login,h.target='_blank';let k=document.createElement('img');k.src=f[g].avatar_url,h.appendChild(k),d('contributors').appendChild(h)}d('contributors-container').style.display='block'})}();var OnePRGame={Map:[{Row:[{Color:'#000'},{Color:'#000'},{Color:'#000'},{Color:'#000'},{Color:'#000'}]},{Row:[{Color:'#000'},{Color:'#888',Text:'GO',Move:'Down',IsStart:!0,OnArrive:function(c){OnePRGame.PlayerAction(c,'PULL REQUEST!'),c.Score+=200}},{Color:'#FF0',Move:'Left',OnArrive:function(c){OnePRGame.PlayerAction(c,'COMMIT!')}},{Color:'#00F',Move:'Left',OnArrive:function(c){OnePRGame.PlayerAction(c,'COMMIT!')}},{Color:'#000'}]},{Row:[{Color:'#000'},{Color:'#0FF',Move:'Down',OnArrive:function(c){OnePRGame.PlayerAction(c,'COMMIT!')}},{Color:'#000'},{Color:'#F00',Move:'Up',OnArrive:function(c){OnePRGame.PlayerAction(c,'COMMIT!')}},{Color:'#000'}]},{Row:[{Color:'#000'},{Color:'#0F0',Move:'Right',OnArrive:function(c){OnePRGame.PlayerAction(c,'COMMIT!')}},{Color:'#FFF',Move:'Right',OnArrive:function(c){OnePRGame.PlayerAction(c,'COMMIT!')}},{Color:'#F0F',Move:'Up',OnArrive:function(c){OnePRGame.PlayerAction(c,'COMMIT!')}},{Color:'#000'}]},{Row:[{Color:'#000'},{Color:'#000'},{Color:'#000'},{Color:'#000'},{Color:'#000'}]}]};OnePRGame.Load=function(){let c=0,d=0;for(let e=0;e<OnePRGame.Map.length;e++)for(let f=0;f<OnePRGame.Map[e].Row.length;f++)OnePRGame.Map[e].Row[f].IsStart&&(c=e,d=f),OnePRGame.Map[e].Row[f].Row=e,OnePRGame.Map[e].Row[f].Column=f;OnePRGame.LoadMap(),OnePRGame.LoadPlayer(c,d),OnePRGame.LoadControls()},OnePRGame.LoadMap=function(){for(let c=0;c<OnePRGame.Map.length;c++){for(let f,e=0;e<OnePRGame.Map[c].Row.length;e++)f=document.createElement('div'),f.className='game-board-tile',f.setAttribute('data-row',OnePRGame.Map[c].Row[e].Row),f.setAttribute('data-column',OnePRGame.Map[c].Row[e].Column),f.style.backgroundColor=OnePRGame.Map[c].Row[e].Color,f.textContent=OnePRGame.Map[c].Row[e].Text,document.getElementsByClassName('game-board')[0].appendChild(f);let d=document.createElement('div');d.className='game-board-row-break',document.getElementsByClassName('game-board')[0].appendChild(d)}},OnePRGame.PlayerAction=function(c,d){let e=document.getElementsByClassName('player')[0];e.removeChild(e.childNodes[1]);let f=document.createElement('div');f.className='action',f.textContent=d,e.appendChild(f)},OnePRGame.LoadPlayer=function(c,d){let e=document.createElement('div');e.className='player',e.textContent='DEV',e.style.top=100*c+'px',e.style.left=100*d+'px';let f=document.createElement('div');f.className='action',f.textContent='INITIAL CHECKIN!',e.appendChild(f),document.getElementsByClassName('game-board')[0].appendChild(e),OnePRGame.Player={Id:1,Row:c,Column:d,Score:0}},OnePRGame.LoadControls=function(){document.getElementsByClassName('controls-move')[0].addEventListener('click',function(){let c=OnePRGame.Map[OnePRGame.Player.Row].Row[OnePRGame.Player.Column].Move;'Up'===c?OnePRGame.Player.Row--:'Down'===c?OnePRGame.Player.Row++:'Left'===c?OnePRGame.Player.Column--:'Right'===c?OnePRGame.Player.Column++:void 0;let d=document.getElementsByClassName('player')[0];d.style.top=100*OnePRGame.Player.Row+'px',d.style.left=100*OnePRGame.Player.Column+'px','undefined'!=typeof OnePRGame.Map[OnePRGame.Player.Row].Row[OnePRGame.Player.Column].OnArrive&&OnePRGame.Map[OnePRGame.Player.Row].Row[OnePRGame.Player.Column].OnArrive(OnePRGame.Player),document.getElementsByClassName('controls-score')[0].textContent=OnePRGame.Player.Score},!1)},function(){OnePRGame.Load()}();var OnePRGame3d={Camera:{Position:[0.5,0.5,2.5],Direction:_MathPI,DrawDistance:4,XFieldOfView:100*_MathPI/180,YFieldOfView:100*_MathPI/180},ScreenWidth:600,ScreenHeight:300,Context:document.getElementsByClassName('game-board')[0].getContext('2d'),Planes:[{Vertices:[[0,0,3],[0,0,2],[1,0,2],[1,0,3]],Color:'#888'},{Vertices:[[0,0,2],[0,0,1],[1,0,1],[1,0,2]],Color:'#0FF'},{Vertices:[[0,0,1],[0,0,0],[1,0,0],[1,0,1]],Color:'#0F0'},{Vertices:[[1,0,0],[1,0,1],[2,0,1],[2,0,0]],Color:'#FFF'},{Vertices:[[2,0,0],[2,0,1],[3,0,1],[3,0,0]],Color:'#F0F'},{Vertices:[[2,0,1],[2,0,2],[3,0,2],[3,0,1]],Color:'#F00'},{Vertices:[[2,0,2],[2,0,3],[3,0,3],[3,0,2]],Color:'#00F'},{Vertices:[[1,0,2],[1,0,3],[2,0,3],[2,0,2]],Color:'#FF0'}]};OnePRGame3d.Load=function(){function c(m){let n=m[0],o=m[2],p=Math.cos(OnePRGame3d.Camera.Direction),q=Math.sin(OnePRGame3d.Camera.Direction),r=o,s=n;return n=_Mathround(100*(s*p+r*q))/100,o=_Mathround(100*(s*-q+r*p))/100,[n,m[1],o]}function d(m){let n={Vertices:[],Color:m.Color};for(let p,o=0;o<m.Vertices.length;o++)p=[m.Vertices[o][0]-OnePRGame3d.Camera.Position[0],m.Vertices[o][1]-OnePRGame3d.Camera.Position[1],m.Vertices[o][2]-OnePRGame3d.Camera.Position[2]],p=c(p),n.Vertices.push(p);return n}function e(m){let n=m[0],o=m[1],p=m[2],q=0==p&&0<n?OnePRGame3d.HalfWidth:0==p&&0>n?-OnePRGame3d.HalfWidth:n*(OnePRGame3d.Camera.XFocalLength/p);0>p&&(q=-6*q);let r=0==p&&0<o?OnePRGame3d.HalfHeight:0==p&&0>o?-OnePRGame3d.HalfHeight:o*(OnePRGame3d.Camera.YFocalLength/p);return 0>p&&(r=-6*r),[q+OnePRGame3d.HalfWidth,-r+OnePRGame3d.HalfHeight]}function f(m){OnePRGame3d.Context.beginPath();for(let n=0;n<m.Vertices.length;n++)0==n?OnePRGame3d.Context.moveTo.apply(OnePRGame3d.Context,m.Vertices[n]):OnePRGame3d.Context.lineTo.apply(OnePRGame3d.Context,m.Vertices[n]);OnePRGame3d.Context.closePath(),OnePRGame3d.Context.fillStyle=m.Color,OnePRGame3d.Context.fill()}function g(m,n){for(var o=m.Vertices[m.Vertices.length-1],p=!1,q=n[2],r=0;r<m.Vertices.length;r++){var s=o[2],t=m.Vertices[r][2];if(s<=q&&t>q||s>=q&&t<q){var v=(m.Vertices[r][0]-o[0])/(t-s),w=o[0]+(q-s)*v;w>n[0]&&(p=!p)}o=m.Vertices[r]}return p}function h(m,n){let o=null,p=null,q=null,r=null;for(let s=0;s<m.Vertices.length;s++)(null==o||_Mathabs(m.Vertices[s][0])<o)&&(o=_Mathabs(m.Vertices[s][0])),0<=m.Vertices[s][2]&&(null==q||m.Vertices[s][2]<q)&&(q=m.Vertices[s][2]);for(let s=0;s<n.Vertices.length;s++)(null==p||_Mathabs(n.Vertices[s][0])<p)&&(p=_Mathabs(n.Vertices[s][0])),0<=n.Vertices[s][2]&&(null==r||n.Vertices[s][2]<r)&&(r=n.Vertices[s][2]);return null==q&&null==r?0:null==q&&null!=r?1:null!=q&&null==r?-1:q<r?1:q==r&&o<p?1:q==r&&o>p?1:q==r&&o==p?0:-1}function k(){OnePRGame3d.Context.fillStyle='rgb(0,0,0)',OnePRGame3d.Context.fillRect(0,0,OnePRGame3d.ScreenWidth,OnePRGame3d.ScreenHeight);let m=[];for(let o=0;o<OnePRGame3d.Planes.length;o++){let p=d(OnePRGame3d.Planes[o]),q={Vertices:[[0,null,0],[-1*OnePRGame3d.Camera.DrawDistance*_Mathtan(OnePRGame3d.Camera.XFieldOfView),null,OnePRGame3d.Camera.DrawDistance],[OnePRGame3d.Camera.DrawDistance*_Mathtan(OnePRGame3d.Camera.XFieldOfView),null,OnePRGame3d.Camera.DrawDistance]]},r=!1;for(let s=0;s<p.Vertices.length;s++)if(g(q,p.Vertices[s])){r=!0;break}r&&m.push(p)}m.sort(h);let n=[];for(let p,o=0;o<m.length;o++){p={Vertices:[],Color:m[o].Color};for(let r,q=0;q<m[o].Vertices.length;q++)r=e(m[o].Vertices[q]),p.Vertices.push(r);n.push(p)}for(let o=0;o<n.length;o++)f(n[o])}OnePRGame3d.HalfWidth=OnePRGame3d.ScreenWidth/2,OnePRGame3d.HalfHeight=OnePRGame3d.ScreenHeight/2,OnePRGame3d.Camera.XFocalLength=OnePRGame3d.HalfWidth/_Mathtan(OnePRGame3d.Camera.XFieldOfView/2),OnePRGame3d.Camera.YFocalLength=OnePRGame3d.HalfHeight/_Mathtan(OnePRGame3d.Camera.YFieldOfView/2);setInterval(function(){k()},50)},OnePRGame3d.MoveForward=function(){OnePRGame3d.Moving=!0;let c=OnePRGame3d.Camera.Direction!=_MathPI&&0!=OnePRGame3d.Camera.Direction,d=OnePRGame3d.Camera.Direction!=_MathPI&&OnePRGame3d.Camera.Direction!=_MathPI/2,e=parseFloat(OnePRGame3d.Camera.Position[c?0:2]),f=e+(d?1:-1);let h=setInterval(function(){if(d&&OnePRGame3d.Camera.Position[c?0:2]>=f||!d&&OnePRGame3d.Camera.Position[c?0:2]<=f){OnePRGame3d.Camera.Position[c?0:2]=f,clearInterval(h);let k='COMMIT!';0.5==f||2.5==f?(OnePRGame3d.TurnLeft(),0.5==OnePRGame3d.Camera.Position[0]&&2.5==OnePRGame3d.Camera.Position[2]&&(document.getElementsByClassName('controls-score')[0].textContent=parseInt(document.getElementsByClassName('controls-score')[0].textContent)+200,k='PULL REQUEST!')):OnePRGame3d.Moving=!1;let l=document.getElementsByClassName('game-board-window')[0];l.removeChild(l.childNodes[4]);let m=document.createElement('div');m.className='action',m.textContent=k,l.appendChild(m)}else OnePRGame3d.Camera.Position[c?0:2]=(parseFloat(OnePRGame3d.Camera.Position[c?0:2])+(d?0.05:-0.05)).toPrecision(3)},50)},OnePRGame3d.TurnLeft=function(){OnePRGame3d.Moving=!0;let c=parseFloat(OnePRGame3d.Camera.Direction),d=c+_MathPI/2;let f=setInterval(function(){OnePRGame3d.Camera.Direction>=d?(OnePRGame3d.Camera.Direction=d,clearInterval(f),OnePRGame3d.Camera.Direction>=2*_MathPI&&(OnePRGame3d.Camera.Direction=0),OnePRGame3d.Moving=!1):OnePRGame3d.Camera.Direction=parseFloat(OnePRGame3d.Camera.Direction)+_MathPI/20},50)},OnePRGame3d.LoadControls=function(){document.getElementsByClassName('controls-move')[0].addEventListener('click',function(){OnePRGame3d.Moving||OnePRGame3d.MoveForward()},!1),document.getElementsByClassName('controls-autoplay')[0].addEventListener('click',function(){setInterval(function(){OnePRGame3d.Moving||document.getElementsByClassName('controls-move')[0].click()},50)},!1)},function(){OnePRGame3d.Load(),OnePRGame3d.LoadControls();let d=document.createElement('div');d.className='action',d.textContent='INITIAL CHECKIN!',document.getElementsByClassName('game-board-window')[0].appendChild(d)}(),((window.gitter={}).chat={}).options={room:'thepracticaldev/1pr',useStyles:!1};function toggleSidebar(){sidebar=document.getElementById('sidebar'),sidebar.className='closed'==sidebar.className?'open':'open'==sidebar.className?'closed':'open'}(function(){let d=function(g){return document.getElementById(g)},e=function(g){let h=new Date-new Date(g),k=_Mathround(h/1e3);if(60>k)return k+' seconds ago';let l=_Mathround(k/60);if(60>l)return l+' minutes ago';let m=_Mathround(l/60);if(24>m)return m+' hours ago';let n=_Mathround(m/24);return n+' days ago'},f=function(g,h){if(g.length<h)return g;let k=g.split(' ');g='';for(let m=0;m<k.length;m++){if(g+=k[m],g.length>=h){g+='...';break}g+=' '}return g};window.api('https://api.github.com/repos/thepracticaldev/1pr/pulls?state=closed',function(g,h){if(g)return console.error(g),d('pr-container').textContent='Could not load latest PR',void(d('pr-container').className='error');for(let k=0;k<h.length;k++)if(null!=h[k].merged_at){let l=h[k];d('pr-last').innerHTML='<a href="'+l.html_url+'">'+f(l.title,50)+'</a> by <a href="'+l.user.html_url+'">'+l.user.login+'</a> merged '+e(l.merged_at);break}d('pr-container').style.display='block'})})(),function(){let d=function(e){return document.getElementById(e)};window.api('https://api.github.com/repos/thepracticaldev/1pr/pulls',function(e,f){if(null!==e)return console.error(e),d('pull-requests').className='error',void(d('pull-requests').textContent='Could not load pull requests');for(let g=0;g<f.length;g++){let h=document.createElement('p'),k=document.createElement('strong'),l=document.createElement('a');l.href=f[g].html_url,l.textContent=f[g].title,k.appendChild(l),h.appendChild(k);let m=document.createElement('br');h.appendChild(m);let n=document.createElement('small');n.textContent='submitted by: '+f[g].user.login,h.appendChild(n);let o=document.createElement('br');h.appendChild(o);let p=document.createElement('span');p.className='g-plusone',p.setAttribute('data-href',f[g].html_url),h.appendChild(p),d('pull-requests-container').appendChild(h),gapi.plusone.go()}})}();
/*!
